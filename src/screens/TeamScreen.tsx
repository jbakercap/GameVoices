  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#121212' }}>
      ...
      {scheduleData.liveGame && (
        <View style={{ marginHorizontal: 16, marginBottom: 12, backgroundColor: '#FFFFFF20', borderRadius: 10, padding: 10, borderWidth: 1, borderColor: '#FFFFFF' }}>
          <Text style={{ color: '#FFFFFF', fontWeight: '700', fontSize: 13 }}>🔴 LIVE: vs {scheduleData.liveGame.opponent}</Text>
      ...
      {activeTab === 'episodes' && (
        <FlatList
          ...
          ListEmptyComponent={() => episodesLoading ? <ActivityIndicator color="#FFFFFF" style={{ marginTop: 40 }} /> : <Text style={{ color: '#888', textAlign: 'center', marginTop: 40 }}>No episodes yet</Text>}
        />
      )}
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              <TouchableOpacity
                ...
                style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, backgroundColor: item.isFollowed ? '#2A2A2A' : '#FFFFFF', borderWidth: item.isFollowed ? 1 : 0, borderColor: '#444' }}>
                <Text style={{ color: item.isFollowed ? '#aaa' : '#fff', fontSize: 12, fontWeight: '600' }}>{item.isFollowed ? '✓' : '+'}</Text>
              </TouchableOpacity>
            </TouchableOpacity>
          )}
          ListEmptyComponent={() => showsLoading ? <ActivityIndicator color="#FFFFFF" style={{ marginTop: 40 }} /> : <Text style={{ color: '#888', textAlign: 'center', marginTop: 40 }}>No shows found</Text>}
        />
      )}
        ...
        <View style={{ flex: 1 }}>
          ...
          {buzzLoading ? (
            ...
            <FlatList
              ...
              ListFooterComponent={fetchingNextBuzz ? <ActivityIndicator color="#FFFFFF" style={{ marginVertical: 16 }} /> : null}
            />
          )}
        </View>
      )}
      ...
      {activeTab === 'roster' && (
        <FlatList
            ...
            <TouchableOpacity
              ...
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  ...
                  {item.isFeatured && <Text style={{ color: '#FFFFFF', fontSize: 10 }}>🔥</Text>}
                </View>
          ...
          ListEmptyComponent={() => rosterLoading ? <ActivityIndicator color="#FFFFFF" style={{ marginTop: 40 }} /> : <Text style={{ color: '#888', textAlign: 'center', marginTop: 40 }}>No roster data</Text>}
        />
      )}
    </View>
  );
