...
const DEFAULT_TEAM_ID = 1; // ESPN team ID for Bruins (default)
...
serve(async (req) => {
  ...
  try {
    ...
    if (gameId) {
      const liveUrl = `https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/summary?event=${gameId}`;
    ...
    const scheduleUrl = `https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/teams/${TEAM_ID}/schedule`;
    ...
    const games: ScheduleGame[] = await Promise.all(
      selectedGames.map(async (event: any) => {
        ...
        if (gameState === 'in' || gameState === 'post') {
          try {
            const summaryUrl = `https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/summary?event=${event.id}`;
